...
func (plugin *Plugin) DelegateAdd(pluginName string, nwCfg *NetworkConfig) (*cniTypesCurr.Result, error) {
	...
	res, err := cniInvoke.DelegateAdd(context.Background(), pluginName, nwCfg.Serialize(), nil)
	if err != nil {
...
func (plugin *Plugin) DelegateDel(pluginName string, nwCfg *NetworkConfig) error {
	...
	err = cniInvoke.DelegateDel(context.Background(), pluginName, nwCfg.Serialize(), nil)
	if err != nil {
